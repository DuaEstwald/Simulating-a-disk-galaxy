...
def fvel(r):
    ind = np.arange(len(m))
    dr = r-coord[ind!=j,i]
    return - (G*mcenter[j]/(np.sqrt(np.sum(r**2))**3.))*r-G*np.sum((m[ind!=j]/(np.sqrt(np.sum(dr**2,axis=1)+eps**2)**3.))*dr.T,axis=1)
...
np.save('cPart3'+str(Npart),coord)
np.save('vPart3'+str(Npart),vel)
...
plt.savefig(str(Npart)+'_Part3.png')
